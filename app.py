...
def respond():
    ...
    if 'help' in text:
        return jsonify({
            ...
            "attachments": [
                ...
                {"text": f"{command} (normal | 1337 | shifted | nicknames | jargon | cats | umlauted | random)"},  # noqa
    ...
    if not text or 'random' in text or text == '':
        ...
        users = [transformation_router(transform_roulette, user)
                 for user in users]
    elif '1337' in text:
...
def transformation_router(index, input):
    transformations = [leet_speak, umlauted, cats, shift_vowels, nickname,
                       jargon]
